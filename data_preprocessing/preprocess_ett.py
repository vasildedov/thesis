...
def create_sliding_windows(data, look_back, horizon, step=1, target='OT', get_exog=False):
    ...
    for i in range(0, len(data) - look_back - horizon + 1, step):
        ...
        if not get_exog:
            ...
            y_window = data.iloc[i + look_back:i + look_back + horizon].values
...
def get_windows(train_data, val_data, test_data, look_back=720, horizon=96, step_train=1, get_exog=False):
    ...
    X_val, y_val = create_sliding_windows(val_data.reset_index(drop=True), look_back, horizon, step_val_test, get_exog=get_exog)
    X_test, y_test = create_sliding_windows(test_data.reset_index(drop=True), look_back, horizon, step_val_test, get_exog=get_exog)
