    ...
    for dataset in ['tourism', 'm3', 'm4', 'etth1', 'etth2']:
        ...
        for freq in freqs:
            ...
            for model_name, model_class, model_kwargs in models:
                ...
                y_true = test['y'].values.reshape(num_series if not full_load else train['unique_id'].nunique(),
                                                  horizon) if not multivariate else y_test.copy()

                if model is None:
                    print(f"No existing model found or retrain mode was enabled for {dataset} - {freq} - {ending}."
                          f" Training a new {model_name}...")
